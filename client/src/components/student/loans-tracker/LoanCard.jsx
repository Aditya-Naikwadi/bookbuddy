...
export const LoanCard = ({
  ...
  if (!eligibility.eligible) {
    if (eligibility.reason === "limit_reached") {
      ...
      disabledReason =
        "Another student has placed a hold reservation on this book.";
    } else {
    ...
    >
      ...
      <div className="flex flex-col items-start sm:items-end justify-center gap-1 shrink-0">
        <Button
          ...
          aria-describedby={
            !eligibility.eligible ? `reason-${loan._id}` : undefined
          }
          className={`text-xs px-4 h-9 font-bold flex items-center gap-1.5 min-w-[120px] ${
            !eligibility.eligible
              ? "opacity-40 cursor-not-allowed border-edge hover:bg-transparent"
              : ""
          }`}
        >
          <RefreshCw
            size={14}
            className={isCurrentRenewing ? "animate-spin" : ""}
          />
