...
export const BadgeGrid = ({ catalog = [], earned = [] }) => {
  ...
  const earnedStickerIds = new Set(earned.map((e) => e.stickerId?.toString() || e.stickerId));

  // Group catalog stickers by category
  const categories = [...new Set(catalog.map((s) => s.category || 'Milestones'))];
    ...
    <div className="space-y-8 w-full">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">
          Unlock achievements by extending check-in streaks and exploring BookBuddy features. You have{' '}
          <span className="text-indigo-600 font-extrabold">{earned.length}</span> of{' '}
          <span className="text-slate-800 font-extrabold">{catalog.length}</span> earned.
        </p>
      </div>
          ...
          <div key={category} className="space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {catStickers.map((sticker) => {
                ...
                const isEarned = earnedStickerIds.has(sticker._id?.toString() || sticker._id);
                ...
                return (
                  <div
                    ...
                    aria-label={`Sticker: ${sticker.name}. Rarity: ${sticker.rarity}. Status: ${
                      isEarned ? 'Unlocked' : `Locked. Criteria: ${sticker.criteria}`
                    }`}
                  >
                    {/* Lock Overlay for locked stickers */}
                    {!isEarned && (
                      <div className="absolute top-3 right-3 text-slate-300" aria-hidden="true">
                        <Lock size={14} />
                      </div>
                    )}
