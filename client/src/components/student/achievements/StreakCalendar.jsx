  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        ...
        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Weekly Log</span>
      </div>
      ...
      <div className="grid grid-cols-7 gap-2 sm:gap-4 select-none">
        {days.map((day) => {
          ...
          if (status === 'checked') {
            ...
            statusClasses = 'bg-emerald-500 text-white border-emerald-500 shadow-sm shadow-emerald-500/25';
            icon = <Check size={14} strokeWidth={3} />;
          } else if (status === 'pending') {
            statusText = 'Pending Check-in';
            statusClasses = 'border-2 border-dashed border-orange-500 text-orange-500 animate-pulse';
          ...
          return (
            ...
            >
              ...
              {/* Day label */}
              <span className={`text-[10px] font-extrabold uppercase tracking-wide ${day.isToday ? 'text-indigo font-black' : 'text-slate-400'}`}>
                {day.dayName}
              </span>
            </div>
          );
