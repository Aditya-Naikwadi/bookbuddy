  ...
  const handleExportImage = () => {
    ...
    try {
      ...
      const expDate = profile.validTill
        ? new Date(profile.validTill).toLocaleDateString(undefined, { month: '2-digit', year: 'numeric' })
        : 'N/A';
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center max-w-sm mx-auto">
        ...
        <p className="text-xs text-muted mt-1 leading-relaxed">
          Verify your connection or login again to reload your digital card credentials.
        </p>
      </div>
    );
  }

  const isBlock = profile?.membershipStatus === 'expired' || profile?.membershipStatus === 'suspended';
    ...
    <div className="w-full flex flex-col items-center space-y-6">
      ...
      {!isOnline && (
        <div className="w-full max-w-md p-3.5 border border-amber-500/20 bg-amber-500/5 rounded-2xl flex items-center gap-3">
          <WifiOff className="text-amber-500 shrink-0 animate-pulse" size={18} />
