...
export const OnboardingTour = ({ user, onCompleteTour, forceStart = false }) => {
  ...
  const currentStep = ONBOARDING_TOUR_STEPS[currentStepIndex] || ONBOARDING_TOUR_STEPS[0];
    ...
    <div className="fixed bottom-8 right-8 z-50 max-w-md w-full bg-slate-900 border border-indigo-500/40 rounded-3xl p-6 shadow-2xl backdrop-blur-xl text-slate-100 animate-slide-up">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 text-indigo-400 font-bold text-xs uppercase tracking-wider">
          ...
          <span>Platform Onboarding Tour ({currentStepIndex + 1}/{ONBOARDING_TOUR_STEPS.length})</span>
        </div>
      ...
      <p className="text-xs text-slate-300 leading-relaxed mb-6">{currentStep.content}</p>
