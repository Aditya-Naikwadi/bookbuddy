...
export const Fines = () => {
        ...
        setFines([
          {
            ...
            overdueDays: 5,
            status: 'unpaid',
            loanId: { bookId: { title: 'Clean Code: Handbook of Software Craftsmanship' } },
          },
          {
            _id: 'fine_demo_2',
            amount: 75.0,
            overdueDays: 10,
            status: 'unpaid',
            loanId: { bookId: { title: 'Introduction to Algorithms (4th Edition)' } },
          },
        ]);
      }
    } catch (err) {
  ...
  const handlePayFines = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const unpaidIds = fines.filter((f) => f.status === 'unpaid').map((f) => f._id);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Settle overdue fines securely using Razorpay PCI-DSS compliant payment gateway.
          </p>
        </div>
      ...
      {processingState === 'processing' && (
        <div className="p-5 rounded-3xl bg-indigo-950/80 border border-indigo-500/40 text-indigo-200 mb-8 flex items-center gap-4 animate-pulse shadow-xl">
          ...
          <div>
            <div className="font-bold text-base text-white">Payment Processing...</div>
            <div className="text-xs text-indigo-300 mt-0.5">
              Reconciling webhook confirmation with server. State will update automatically once verified.
            </div>
          </div>
        </div>
      )}
      ...
      {processingState === 'confirmed' && (
        <div className="p-5 rounded-3xl bg-emerald-950/80 border border-emerald-500/40 text-emerald-200 mb-8 flex items-center gap-4 shadow-xl">
          ...
          <div>
            <div className="font-bold text-base text-white">Payment Confirmed & Verified!</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
        <div className="lg:col-span-1 bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col justify-between">
          ...
          <button
            disabled={totalUnpaidAmount <= 0 || processingState === 'processing'}
          ...
          >
            ...
            <span>{processingState === 'processing' ? 'Processing...' : `Pay ₹${totalUnpaidAmount.toFixed(2)} Now`}</span>
          </button>
        </div>
        ...
        <div className="lg:col-span-2 bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <Clock className="w-5 h-5 text-indigo-400" /> Fine Itemization ({fines.length})
          </h2>

          {loading ? (
            <div className="space-y-3">
              {[1, 2].map((i) => (
                <div key={i} className="h-16 bg-slate-800/40 rounded-2xl animate-pulse"></div>
              ))}
            </div>
          ) : fines.length === 0 ? (
            ...
            <div className="divide-y divide-slate-800/80">
              {fines.map((item) => {
                ...
                const bookTitle = (item.loanId || {}).bookId?.title || 'Library Resource';

                return (
                  <div key={item._id} className="py-4 flex items-center justify-between gap-4">
                    <div>
                      <div className="font-bold text-white text-sm">{bookTitle}</div>
