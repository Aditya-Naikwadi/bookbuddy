...
const GeneralDashboardHome = () => {
  ...
  const filteredSuggestions = searchQuery.trim()
    ? [...popularBooks, ...newArrivals]
        .filter(
          (b, index, self) =>
            self.findIndex((item) => (item._id || item.id) === (b._id || b.id)) === index
        )
        .filter(
          (b) =>
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && searchQuery.trim()) {
                  navigate(`/general-dashboard/search?q=${encodeURIComponent(searchQuery)}`);
              ...
              {filteredSuggestions.map((book) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-slate-500 truncate">By {book.author}</div>
                  </div>
        ...
        <div className="bg-indigo-900 text-white p-3 px-4 rounded-2xl flex items-center justify-between gap-3 shadow-md flex-shrink-0 animate-in fade-in duration-300">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-xs font-medium">
              <span className="font-bold text-white">{t('welcomeTitle')}: </span>
              {t('welcomeSubtitle')}
            </p>
          </div>
          <button
              ...
              <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-900">{t('libraryHours')}</span>
                  </div>
                  <span
                  ...
                  >
                    {isOpen && !libraryHours.isClosedToday ? t('openNow') : t('closed')}
                  </span>
                </div>
            ...
            <DashboardErrorBoundary widgetName="Catalog Metrics">
              ...
              >
                <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-500 block">{t('totalCatalogBooks')}</span>
                    <span className="text-2xl font-bold text-slate-900">
                      {(stats?.totalCatalogBooks || stats?.totalBooks || 0).toLocaleString()}
                    </span>
                    <span className="text-[10px] text-emerald-600 font-semibold block mt-0.5">
                      +{(stats?.addedThisMonth || stats?.newArrivalsCount || 0)} {t('addedThisMonth')}
                    </span>
                  </div>
            ...
            <DashboardErrorBoundary widgetName="Category Breakdown">
              <BookDataState isLoading={isLoading} isError={isError} onRetry={handleRefresh}>
                <div className="bg-white p-3.5 rounded-2xl border border-slate-200/80 shadow-sm flex-1 flex flex-col justify-between">
                  ...
                  <DonutChart data={stats?.categoryBreakdown || []} size={84} strokeWidth={12} />
                </div>
              </BookDataState>
            </DashboardErrorBoundary>
          </div>
            ...
            <DashboardErrorBoundary widgetName="Popular Carousel">
              <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm flex-1 flex flex-col justify-between min-h-0 overflow-hidden">
                <div className="flex items-center justify-between mb-3 flex-shrink-0">
                  <div>
                    <h2 className="text-sm font-bold text-slate-900">{t('popularThisWeek')}</h2>
      ...
      {selectedBook && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-5 shadow-2xl border border-slate-100 relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-slate-500 mb-3">By {selectedBook.author}</p>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5 text-xs">
              ...
              <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/60">
                ...
                <span className="font-bold text-slate-900 text-xs flex items-center gap-1">
                  ...
                  <span className="truncate">{selectedBook.shelfLocation || 'Main Stacks'}</span>
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <span>{isBookmarked(selectedBook._id || selectedBook.id) ? 'Saved' : 'Bookmark Item'}</span>
              </button>
