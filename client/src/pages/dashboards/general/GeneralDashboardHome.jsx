...
const GeneralDashboardHome = () => {
  ...
  return (
    <div className="flex flex-col min-h-full max-w-7xl mx-auto p-3 sm:p-4 gap-4 font-sans pb-10 text-slate-100">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-start gap-4 md:gap-5 bg-slate-900 p-4 px-5 rounded-2xl border border-slate-800 shadow-xl flex-shrink-0">
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="p-2.5 bg-indigo-950/80 text-indigo-400 border border-indigo-800/60 rounded-xl flex-shrink-0">
            <BookOpen className="w-5 h-5" />
          </div>
          <div>
            <h1 className="text-lg sm:text-xl font-bold text-slate-100 tracking-tight leading-tight">
        ...
        <div className="relative flex-1 max-w-md">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-xs bg-slate-950 border border-slate-700/80 rounded-xl text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500 transition-all"
            />
            {searchQuery && (
              <button
          ...
          {showSuggestions && filteredSuggestions.length > 0 && (
            <div className="absolute top-full mt-1.5 left-0 right-0 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
              <div className="p-2.5 border-b border-slate-800 text-[10px] font-bold text-slate-400 uppercase tracking-wider bg-slate-950/60">
              ...
              {filteredSuggestions.map((book) => (
                <button
                  ...
                  className="w-full px-3 py-2.5 text-left hover:bg-slate-800/80 flex items-center justify-between gap-2 transition-colors border-b last:border-0 border-slate-800/60"
                >
                  ...
                  <span className="text-[9px] font-semibold px-2 py-0.5 rounded-md bg-indigo-950 text-indigo-300 border border-indigo-800/60 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 flex-shrink-0 self-end md:self-auto ml-auto">
          <button
            ...
            className="p-2 rounded-xl border border-slate-700/80 bg-slate-950 text-slate-300 hover:bg-indigo-600 hover:text-white hover:border-indigo-500 transition-all disabled:opacity-50"
          >
            <RefreshCw
      ...
      <div className="flex sm:hidden bg-slate-900 p-1 rounded-xl gap-1 flex-shrink-0 text-xs font-semibold text-slate-400 border border-slate-800">
        <button
      ...
      {bookmarks.length === 0 && (
        <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 text-white p-3.5 px-4 rounded-2xl border border-indigo-500/30 flex items-center justify-between gap-3 shadow-xl flex-shrink-0 animate-in fade-in duration-300">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-3 min-h-0 overflow-hidden">
        ...
        {(activeTab === "overview" || activeTab === "notices") && (
          <div className="md:col-span-4 flex flex-col gap-3 min-h-0 overflow-y-auto pr-1">
            ...
            <DashboardErrorBoundary widgetName="Library Hours">
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-xl flex flex-col justify-between">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {newArrivals.slice(0, 3).map((item, i) => (
                      <button
                        ...
                        className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 text-slate-100 text-[10px] font-bold h-16 flex flex-col justify-between hover:border-indigo-500/60 hover:bg-slate-800/80 transition-all shadow-sm text-left relative overflow-hidden group"
                      >
                        <span className="line-clamp-2 leading-tight group-hover:text-indigo-300 transition-colors">
                          {item.title}
                        </span>
            ...
            <DashboardErrorBoundary widgetName="Category Breakdown">
              <BookDataState
                ...
                onRetry={handleRefresh}
              >
                <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-xl flex-1 flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-slate-100">
        ...
        {(activeTab === "overview" || activeTab === "actions") && (
          <div className="md:col-span-8 flex flex-col gap-3 min-h-0 overflow-hidden">
            ...
            <div className="bg-slate-900 p-3 rounded-2xl border border-slate-800 shadow-xl flex-shrink-0">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <button
                  onClick={() => navigate("/general-dashboard/search")}
                  className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 hover:bg-slate-800 hover:border-indigo-500/50 transition-all text-xs font-bold shadow-md"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 hover:bg-slate-800 hover:border-emerald-500/50 transition-all text-xs font-bold shadow-md"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 hover:bg-slate-800 hover:border-purple-500/50 transition-all text-xs font-bold shadow-md"
                >
                ...
                user?.role === "super-admin" ? (
                  <button
                    ...
                    className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 hover:bg-slate-800 hover:border-amber-500/50 transition-all text-xs font-bold shadow-md"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 hover:bg-slate-800 hover:border-sky-500/50 transition-all text-xs font-bold shadow-md"
                  >
            ...
            <DashboardErrorBoundary widgetName="Popular Carousel">
              <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-xl flex-1 flex flex-col justify-between min-h-0 overflow-hidden">
                <div className="flex items-center justify-between mb-3 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="p-1.5 rounded-xl border border-slate-700 bg-slate-950 hover:bg-slate-800 text-slate-300 transition-colors"
                    >
                  ...
                  >
                    {popularBooks.map((book) => {
                      ...
                      return (
                        <div
                          key={book._id || book.id}
                          className="w-64 flex-shrink-0 bg-slate-950/90 rounded-2xl border border-slate-800/90 p-3.5 snap-start hover:shadow-2xl hover:border-indigo-500/50 transition-all duration-200 flex flex-col justify-between group"
                        >
                          <div>
                            ...
                            <h3 className="font-bold text-slate-100 text-xs line-clamp-1 group-hover:text-indigo-400 transition-colors">
                              {book.title}
                            </h3>
                            <p className="text-[11px] text-slate-400">
                              {book.author}
      ...
      {selectedBook && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-3xl max-w-md w-full p-5 shadow-2xl border border-slate-800 relative animate-in fade-in zoom-in-95 duration-200 text-slate-100">
            <button
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-300 bg-indigo-950 border border-indigo-800/60 px-2.5 py-1 rounded-lg">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="py-2.5 px-3 rounded-xl border border-slate-700 bg-slate-800 text-xs font-bold text-slate-200 hover:bg-slate-700 transition-colors"
              >
                Catalog Search
              </button>
            </div>
