...
const GeneralEResources = () => {
  ...
  const categoryStats = useMemo(() => {
    ...
    const pastPapers = allList.filter((i) => i.category === 'Past Papers').length;

    return [
      { label: 'E-Books', value: ebooks, icon: BookOpen, colorClass: 'text-indigo-600', bgBadgeClass: 'bg-indigo-50 text-indigo-700' },
      { label: 'Journals', value: journals, icon: Globe, colorClass: 'text-emerald-600', bgBadgeClass: 'bg-emerald-50 text-emerald-700' },
      { label: 'Databases', value: databases, icon: Database, colorClass: 'text-purple-600', bgBadgeClass: 'bg-purple-50 text-purple-700' },
      { label: 'Past Papers', value: pastPapers, icon: FileCheck, colorClass: 'text-amber-600', bgBadgeClass: 'bg-amber-50 text-amber-700' },
    ];
  }, [allList]);
        ...
        emptyState={
          <div className="bg-white p-8 sm:p-12 rounded-3xl border border-slate-200/80 shadow-sm text-center space-y-4 max-w-md my-auto">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              No public e-resources match your active category and keyword filter. Try resetting filters.
            </p>
            <button
            ...
            >
              <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-lg border ${
                        isOpenAccess ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-xl border transition-all ${
                        bookmarked ? 'bg-amber-500 text-white border-amber-500 shadow-xs' : 'bg-slate-50 text-slate-400 border-slate-200 hover:text-slate-700'
                      }`}
                    >
                ...
                <div className="flex items-center gap-2 text-[10px] font-semibold text-slate-400 mb-1">
                  ...
                  <span>•</span>
                  <span className="text-indigo-600 font-bold">{item.format}</span>
                </div>
                ...
                <p className="text-xs text-slate-600 leading-relaxed line-clamp-3 mb-3">{item.description}</p>
              </div>

              <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2 mt-auto">
                <span className="text-[10px] text-slate-400 font-medium truncate">Source: {item.source}</span>
