  ...
  const fetchIncomingQueue = async () => {
    try {
      ...
      if (res.ok && data.success) {
        ...
        setErrorMsg(
          data.message || "Failed to fetch incoming share requests queue.",
        );
      }
    } catch (err) {
  ...
  const handleStatusUpdate = async (requestId, newStatus) => {
    try {
      ...
      if (res.ok && data.success) {
        ...
        alert(
          data.message ||
            "Status transition rejected by state machine or authorization rules.",
        );
      }
    } catch (err) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 text-indigo-400 font-semibold text-xs uppercase tracking-wider mb-1">
            <ShieldCheck className="w-4 h-4 text-indigo-400" /> Inter-Library
            Loan Queue
          </div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
            <Share2 className="w-8 h-8 text-indigo-400" /> Incoming Share
            Requests
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Review and fulfill cross-college resource requests originating from
        ...
        {loading ? (
          <div className="space-y-4 py-8">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-20 bg-slate-800/40 rounded-2xl animate-pulse"
              ></div>
            ))}
          </div>
        ) : requests.length === 0 ? (
          <div className="text-center py-16 px-4">
            ...
            <h3 className="text-lg font-bold text-white mb-1">
              No Incoming Share Requests
            </h3>
            <p className="text-slate-400 text-sm max-w-md mx-auto">
              There are currently no pending inter-library loan requests for
              your college's resources.
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-white font-bold text-base">
                          {requestingCollege.name || "Partner College"} (
                          {requestingCollege.shortName || "CAMPUS"})
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-xs text-slate-300">
                        ...
                        <strong className="text-white">
                          {requestingStudent.name || "Student"}
                        </strong>{" "}
                        ({requestingStudent.email || "N/A"})
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 self-end lg:self-center">
                    {reqItem.status === "requested" && (
                      <>
                        <button
                          ...
                          onClick={() =>
                            handleStatusUpdate(reqItem._id, "approved")
                          }
                        ...
                        <button
                          disabled={updatingId === reqItem._id}
                          onClick={() =>
                            handleStatusUpdate(reqItem._id, "rejected")
                          }
                    ...
                    {reqItem.status === "approved" && (
                      <button
                        disabled={updatingId === reqItem._id}
                        onClick={() =>
                          handleStatusUpdate(reqItem._id, "in_transit")
                        }
                    ...
                    {reqItem.status === "in_transit" && (
                      <button
                        disabled={updatingId === reqItem._id}
                        onClick={() =>
                          handleStatusUpdate(reqItem._id, "fulfilled")
                        }
