...
export default function DigitalAssetsDesk() {
  ...
  const moderateMutation = useMutation({
    mutationFn: ({ id, status }) =>
      collegeAdminApi.moderateEResource(id, { status }),
    onSuccess: () => {
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 py-6 text-slate-100">
      <div className="border-b border-slate-800 pb-6">
        ...
        <p className="text-sm text-slate-400 mt-1">
          Review student and faculty digital document submissions before
          publishing to campus catalog.
        </p>
      </div>
          ...
          {pending.map((res) => (
            ...
            >
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-800">
                <button
                  onClick={() =>
                    moderateMutation.mutate({ id: res._id, status: "rejected" })
                  }
                ...
                <button
                  onClick={() =>
                    moderateMutation.mutate({ id: res._id, status: "approved" })
                  }
