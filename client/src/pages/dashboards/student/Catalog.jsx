...
import { Bell, Filter, Search, BookOpen, AlertCircle, RefreshCw } from 'lucide-react';
...
const CatalogSkeleton = () => (
  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6" aria-hidden="true">
    {[...Array(10)].map((_, i) => (
      <div key={i} className="bg-surface/30 rounded-xl border border-edge/50 overflow-hidden flex flex-col p-4 space-y-4 animate-pulse">
...
const Catalog = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['catalog', { query: debouncedQuery, category, format, availability, sortBy, page }],
  ...
  const ariaAnnouncement = isLoading ? "Loading catalog..." : `Found ${pagination.total} books${term}.`;

  // Place a reservation hold on a checked-out book
  const handlePlaceHold = async (bookId) => {
    try {
      const res = await apiClient.post('/dashboards/student/reservations', { bookId });
      setQueueMessage({
        type: 'success',
      ...
      setQueueMessage({
        type: 'error',
        text: err.response?.data?.message || 'Failed to place hold. Please try again.',
      });
    }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4 md:px-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-edge/20 pb-4">
        <div>
          <h1 className="text-3xl font-serif font-bold text-ink">Library Catalog</h1>
          <p className="text-sm text-muted">Discover e-books, physical assets, and learning resources.</p>
        </div>
      ...
      <div 
        ...
        className={`grid grid-cols-1 sm:grid-cols-4 gap-4 p-5 glass-panel bg-surface/20 border border-edge/30 rounded-xl transition-all duration-300 ${
          showFilters ? 'opacity-100 max-h-[500px] visible' : 'opacity-0 max-h-0 overflow-hidden invisible py-0 border-none'
        }`}
        aria-hidden={!showFilters}
      >
        <div>
          <label htmlFor="format-filter" className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">Format</label>
          <select
            id="format-filter"
            value={format}
        ...
        <div>
          <label htmlFor="availability-filter" className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">Availability</label>
          <select
        ...
        <div>
          <label htmlFor="category-filter" className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">Subject / Category</label>
          <select
            id="category-filter"
            value={category}
        ...
        <div>
          <label htmlFor="sort-filter" className="block text-xs font-semibold text-muted uppercase tracking-wider mb-2">Sort By</label>
          <select
            id="sort-filter"
            value={sortBy}
      ...
      {(debouncedQuery || format !== 'all' || availability !== 'all' || category !== 'all') && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {availability !== 'all' && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs bg-ember/10 border border-ember/20 text-ink">
              Availability: {availability === 'available' ? 'Available Now' : 'Checked Out'}
              <button 
      ...
      {isError && (
        <div className="p-6 bg-danger/10 border border-danger/20 text-danger rounded-xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm">We are unable to load library catalog. Please check your network and try again.</p>
          </div>
        </div>
      )}
        ...
        <>
          {books.length === 0 ? (
            <div className="text-center py-16 p-6 glass-panel border border-edge/30 rounded-xl flex flex-col items-center max-w-lg mx-auto">
              ...
              <h3 className="text-lg font-bold text-ink">No results match your search</h3>
              <p className="text-sm text-muted mt-2">
                Try expanding your search query, checking for spelling mistakes, or adjusting your filter settings.
              </p>
              <Button 
                variant="ghost" 
                className="mt-6"
                  ...
                  <div className="h-44 bg-surface/50 w-full relative flex items-center justify-center border-b border-edge/35 overflow-hidden">
                    {book.coverImage ? (
                      ...
                      <div className="flex flex-col items-center text-muted/30 select-none">
                        <BookOpen size={40} />
                        <span className="text-[10px] mt-2 font-semibold">NO COVER</span>
                      </div>
                    )}
                    ...
                    <span className={`absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm ${
                      ...
                      {book.copiesAvailable > 0 ? `Available (${book.copiesAvailable})` : 'Out of Stock'}
                    </span>
                  </div>

                  <div className="p-4 flex-1 flex flex-col">
                    <span className="text-[10px] font-semibold text-muted uppercase tracking-wider mb-1">{book.category}</span>
                    <h3 className="font-bold text-ink line-clamp-2 leading-tight mb-1" title={book.title}>{book.title}</h3>
                    <p className="text-xs text-muted mb-4 line-clamp-1">{book.author}</p>
          ...
          {pagination.pages > 1 && (
            <div className="flex items-center justify-between border-t border-edge/20 pt-6 mt-4">
              <span className="text-xs text-muted">
                Showing Page <strong>{page}</strong> of <strong>{pagination.pages}</strong> ({pagination.total} total items)
              </span>
