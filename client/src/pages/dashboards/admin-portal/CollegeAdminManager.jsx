...
import { Shield, UserPlus, AlertTriangle, Check, RefreshCw, Globe } from 'lucide-react';
...
const CollegeAdminManager = () => {
  ...
  const handleCreateCollege = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'College registered successfully as PENDING.' });
      setCollegeName('');
      setCollegeCode('');
      setCollegeEmail('');
      setCollegePhone('');
      ...
      setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to register college.' });
    }
  };

  const handleCreateAdmin = async (e) => {
    e.preventDefault();
    if (!adminName || !adminEmail || !adminStudentId || !adminPassword || !selectedCollegeId) {
      setMessage({ type: 'error', text: 'All fields are required to provision an admin.' });
      return;
    }
    try {
      ...
      setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to provision admin.' });
    }
  };

  const handleStatusChange = async (collegeId, newStatus) => {
    const confirmMsg = `Are you sure you want to transition this college to ${newStatus.toUpperCase()}?` + 
      (newStatus === 'archived' ? ' WARNING: This is a terminal state and cannot be undone.' : '');
      
    if (!window.confirm(confirmMsg)) return;

    try {
      await adminApi.updateCollegeStatus(collegeId, newStatus);
      setMessage({ type: 'success', text: `College status updated to ${newStatus}.` });
      fetchData();
    } catch (err) {
      setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to update college status.' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-serif font-bold text-slate-900">College & Admin Onboarding</h1>
          <p className="text-slate-600">Provision colleges, create admins, and enforce administrative lifecycles.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-fit space-y-4">
          <div className="flex items-center gap-2 border-b pb-3">
            ...
            <h2 className="text-lg font-bold text-slate-900">1. Onboard College</h2>
          </div>
          <form onSubmit={handleCreateCollege} className="space-y-3">
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white text-sm font-medium py-2 rounded-lg hover:bg-indigo-700 transition-colors">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-fit space-y-4">
          <div className="flex items-center gap-2 border-b pb-3">
            ...
            <h2 className="text-lg font-bold text-slate-900">2. Provision Admin</h2>
          </div>
          <form onSubmit={handleCreateAdmin} className="space-y-3">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Assigned College</label>
              <select 
              ...
              >
                {colleges.map((c) => (
                  <option key={c._id} value={c._id} disabled={c.status === 'archived'}>
                    {c.name} ({c.code}) {c.status === 'archived' ? '[ARCHIVED]' : ''}
                  </option>
                ))}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Admin ID (studentId)</label>
              <input 
                type="text" 
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-fit space-y-4">
          <div className="flex items-center gap-2 border-b pb-3">
            ...
            <h2 className="text-lg font-bold text-slate-900">3. College Status Control</h2>
          </div>
          <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1">
            {colleges.map((c) => (
              <div key={c._id} className="p-3 border rounded-lg hover:shadow-xs transition-shadow space-y-2">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-xl font-bold mb-4 text-slate-900">Provisioned College Admins</h2>
        <div className="overflow-x-auto">
          {admins.length === 0 ? (
            <div className="text-center py-6 text-slate-500">No college admins registered yet.</div>
          ) : (
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="bg-slate-50 border-y border-slate-200 text-slate-900">
                <tr>
                  ...
                  <th className="p-3 font-semibold rounded-tr-lg">College Assigned</th>
                </tr>
              </thead>
              <tbody>
                {admins.map((admin) => (
