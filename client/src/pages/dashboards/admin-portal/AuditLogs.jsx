...
import { FileSearch, Clock, Filter, ChevronLeft, ChevronRight, Eye, RefreshCw } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => { setSelectedCollegeId(e.target.value); setPage(1); }}
          ...
          <div>
            ...
            >
              ...
              <option value="college.status_change">college.status_change</option>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-900">System Events ({totalLogs})</h2>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-slate-500">No events match the filter parameters.</div>
          ) : (
            <table className="w-full text-left text-sm text-slate-600 font-mono">
              <thead className="bg-slate-50 border-y border-slate-200 text-slate-900 font-sans">
                <tr>
                  ...
                  <th className="p-3 font-semibold rounded-tr-lg text-right">Details</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
                  <tr key={log._id} className="border-b border-slate-100 hover:bg-slate-50 text-xs">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold font-sans uppercase ${
                        log.actorRole === 'super-admin' ? 'bg-indigo-100 text-indigo-800' : 'bg-blue-100 text-blue-800'
                      }`}>
                        {log.actorRole}
                      </span>
                    </td>
                    <td className="p-3 font-bold font-sans text-slate-900">
                      {log.actorId ? `${log.actorId.name} (${log.actorId.studentId})` : 'System / Seed'}
                    </td>
                    <td className="p-3 font-bold font-sans text-slate-800">{log.action}</td>
                    <td className="p-3 text-slate-500">{log.ipAddress || 'N/A'}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4 font-sans">
          <div className="bg-white rounded-xl shadow-lg border max-w-lg w-full p-6 space-y-4">
            <div className="flex justify-between items-center border-b pb-3">
              <h3 className="font-bold text-lg text-slate-900">Event Metadata Payload</h3>
              <button
            ...
            <div className="space-y-2 text-sm">
              ...
              <div>
                <span className="font-semibold text-slate-500">Actor Details: </span>
                <span>{selectedLog.actorId ? `${selectedLog.actorId.name} (${selectedLog.actorId.email})` : 'System'}</span>
              </div>
              {selectedLog.collegeId && (
                <div>
                  <span className="font-semibold text-slate-500">College: </span>
                  <span>{selectedLog.collegeId.name} ({selectedLog.collegeId.code})</span>
                </div>
              )}
            ...
            <div className="space-y-1">
              <span className="font-semibold text-slate-500 text-sm">Metadata Diff:</span>
