      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="text-success text-sm font-bold flex items-center">Healthy</span>
          </div>
          <h3 className="text-slate-500 text-sm font-medium">Total Registered Colleges</h3>
          <p className="text-3xl font-bold text-slate-900 mt-1">{stats?.totalColleges || 0}</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <h3 className="text-slate-500 text-sm font-medium">Total Storage Used</h3>
          <p className="text-3xl font-bold text-slate-900 mt-1">{formatStorage(stats?.storageUsageBytes)}</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <p className="text-3xl font-bold text-slate-900 mt-1">{stats?.activeLoans || 0}</p>
        </div>
      </div>

      {/* College Instance Health Table */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h2 className="text-xl font-bold mb-4 text-slate-900">College Instance Health</h2>
        <div className="overflow-x-auto">
          {colleges.length === 0 ? (
            <div className="text-center py-6 text-slate-500">No colleges registered on the platform.</div>
          ) : (
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody>
                {colleges.map((college) => (
                  <tr key={college._id} className="border-b border-slate-100 hover:bg-slate-50">
                    <td className="p-3 font-bold text-slate-900">{college.name}</td>
                    <td className="p-3 font-mono text-xs">{college.code}</td>
                    <td className="p-3">{(college.metrics?.activeStudents || 0).toLocaleString()}</td>
                    <td className="p-3">{formatStorage(college.metrics?.storageUsageBytes || 0)}</td>
                    <td className="p-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${getStatusColor(college.status)}`}>
                        {college.status}
                      </span>
                    </td>
                    <td className="p-3">
                      <div className="text-xs text-slate-500">
                        {college.contactEmail ? college.contactEmail : 'No contact email'}
                      </div>
                    </td>
                  </tr>
                ))}
