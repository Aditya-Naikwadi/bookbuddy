...
export const useCheckIn = () => {
  ...
  const checkInMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      if (prevStreak) {
        queryClient.setQueryData(['streak', 'me'], {
          ...
          longestStreak: Math.max(prevStreak.longestStreak, prevStreak.currentStreak + 1),
          todayComplete: true,
        });
      }
    ...
    onSuccess: (data) => {
      ...
      setAnnouncement(`Check-in successful! Your daily reading streak is now ${newStreakCount} days.`);
    ...
    onError: (err, variables, context) => {
      ...
      setAnnouncement('Failed to check in. Please verify your connection and try again.');
    },
  });
  ...
  const repairMutation = useMutation({
    ...
    onError: () => {
      setAnnouncement('Failed to repair streak. Make sure you have freezes available.');
    },
  });
