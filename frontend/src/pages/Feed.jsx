  ...
  const handleRsvp = async (postId) => {
    try {
      ...
      if (data.success && data.data) {
        setPosts((prev) =>
          prev.map((p) =>
            p._id === postId ? { ...p, rsvps: data.data.rsvps } : p,
          ),
        );
      }
    } catch (err) {
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-slate-900/30 rounded-3xl border border-slate-800/60 p-8">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              Check back soon for new announcements or events posted by college
              admins.
            </p>
          </div>
        ) : viewMode === "calendar" ? (
            ...
            {posts
              ...
              .map((post) => {
                ...
                const isRsvped =
                  currentUserId &&
                  post.rsvps?.some(
                    (r) => r.userId && r.userId.toString() === currentUserId,
                  );
                    ...
                    <div className="mt-5 p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 flex flex-wrap items-center justify-between gap-4">
                      ...
                      >
                        {rsvpLoadingId === post._id ? (
                          ...
                          <>
                            <UserCheck className="w-4 h-4" /> Attending (Toggle
                            Off)
                          </>
                        ) : (
                          <>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                <Megaphone className="w-5 h-5 text-indigo-400" /> Post Campus
                Update
              </h3>
              <button
              ...
              {postForm.type === "event" && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPostForm({ ...postForm, eventDate: e.target.value })
                    }
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
