            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-3 text-xs sm:text-sm text-slate-300">
                ...
                <span className="flex items-center gap-1 font-semibold text-emerald-400">
                  <Award className="w-4 h-4" /> {earnedBadges.length} Badges
                  Earned
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-sm font-semibold flex items-center gap-1.5 mt-0.5">
                {user?.isLeaderboardVisible !== false ? (
                  <>
                    <Eye className="w-4 h-4 text-emerald-400" /> Visible on
                    Leaderboard
                  </>
                ) : (
                  <>
                    <EyeOff className="w-4 h-4 text-rose-400" /> Hidden
                    (Anonymous)
                  </>
                )}
              </p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {allBadges.map((badge) => {
              ...
              const earnedInfo = earnedBadges.find(
                (b) => (b.badgeKey || b.key) === badge.key,
              );
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                    <span>
                      {isEarned ? (
                        ...
                        <span>
                          Requires {badge.criteria?.threshold || 1}{" "}
                          {badge.criteria?.type || "action"}
                        </span>
                      )}
                    </span>
                    {earnedInfo?.earnedAt && (
