  ...
  const fetchCrossCollegeCatalog = async (query = "", showLoading = false) => {
    try {
      ...
      const res = await fetch(
        `/api/v1/catalog/cross-college?q=${encodeURIComponent(query)}`,
        {
          headers: {
    ...
    try {
      ...
      const res = await fetch("/api/v1/share-requests", {
        ...
        body: JSON.stringify({
          ...
          resourceType:
            selectedResource.type === "pdf" || selectedResource.type === "epub"
              ? "eresource"
              : "book",
        }),
      });
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Explore and request physical books & e-resources opted into
          inter-library sharing by partner colleges.
        </p>
      </div>
      ...
      <div className="mb-12">
        <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
          <BookOpen className="w-5 h-5 text-indigo-400" /> Shareable Collection
          ({allItems.length})
        </h2>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-44 bg-slate-900/60 rounded-3xl animate-pulse"
              ></div>
            ))}
          </div>
        ) : allItems.length === 0 ? (
          <div className="text-center py-12 bg-slate-900/40 rounded-3xl border border-slate-800 p-6">
            ...
            <p className="text-slate-400 text-xs">
              No partner college has opted in shareable items matching your
              search query.
            </p>
          </div>
        ) : (
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-white mb-2">
              Request from{" "}
              {(selectedResource.collegeId || {}).name || "Partner College"}?
            </h3>

            <p className="text-slate-400 text-xs mb-6">
              You are submitting an Inter-Library Loan request for{" "}
              <strong className="text-white">"{selectedResource.title}"</strong>
              . The owning college administrator will review your request.
            </p>
      ...
      {myRequests.length > 0 && (
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-xl">
          <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <Clock className="w-5 h-5 text-indigo-400" /> My Inter-Library Share
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="text-sm font-bold text-white">
                    Request from{" "}
                    {(reqItem.owningCollegeId || {}).name || "Partner College"}
                  </div>
                </div>
