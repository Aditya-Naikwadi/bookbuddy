...
export const Downloads = () => {
  ...
  const loadOfflineItems = async (showLoading = false) => {
    try {
      ...
      const totalBytes = offlineList.reduce(
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 text-indigo-400 font-semibold text-xs uppercase tracking-wider mb-1">
            <ShieldCheck className="w-4 h-4 text-emerald-400" /> DRM-Light
            Offline Access
          </div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
            <HardDrive className="w-8 h-8 text-indigo-400" /> Offline Downloads
            Manager
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Manage your IndexedDB-stored e-books and papers for reading without
            network access.
          </p>
        </div>
      ...
      <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-xl">
        <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
          <BookOpen className="w-5 h-5 text-indigo-400" /> Downloaded Items (
          {items.length})
        </h2>
        ...
        {loading ? (
          <div className="space-y-4 py-8">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-16 bg-slate-800/50 rounded-2xl animate-pulse"
              ></div>
            ))}
          </div>
        ) : items.length === 0 ? (
          <div className="text-center py-16 px-4">
            ...
            <p className="text-slate-400 text-sm max-w-md mx-auto mb-6">
              You haven't downloaded any e-resources for offline reading yet.
              Visit the library e-resources page to download books.
            </p>
            <button
