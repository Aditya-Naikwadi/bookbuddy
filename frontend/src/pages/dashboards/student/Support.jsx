...
const Support = () => {
  ...
  const isAnySubmitting =
    isSubmittingSuggestion || isSubmittingComplaint || isSubmittingFeedback;
      ...
      <div className="border-b border-slate-200 pb-4 text-center sm:text-left flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Request new library resources, lodge complaints, or send general
            feedback to college admins.
          </p>
        </div>
          ...
          <div className="md:col-span-2 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm relative">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-extrabold text-sm">
                    {successInfo.type} Submitted!
                  </span>
                </div>
                <p className="text-[10px] text-emerald-700 leading-relaxed">
                  Your request was registered successfully. Reference code for
                  tracking:{" "}
                  <strong className="font-mono text-slate-800 select-all">
                    {successInfo.refId}
                  </strong>
                  . You can monitor its review status in the "My Submissions"
                  logs.
                </p>
              </div>
            )}
            ...
            {subForm === "suggestion" && (
              <PurchaseSuggestionForm
                ...
                onFieldChange={(field, val) =>
                  handleFieldChange("suggestion", field, val)
                }
            ...
            {subForm === "complaint" && (
              <ComplaintForm
                ...
                isPending={isAnySubmitting}
                onFieldChange={(field, val) =>
                  handleFieldChange("complaint", field, val)
                }
            ...
            {subForm === "feedback" && (
              <FeedbackForm
                ...
                isPending={isAnySubmitting}
                onFieldChange={(field, val) =>
                  handleFieldChange("feedback", field, val)
                }
