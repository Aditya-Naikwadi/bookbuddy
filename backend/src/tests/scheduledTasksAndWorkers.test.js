/**
 * Consolidated Suite: scheduled Tasks And Workers
 * Merged from:
 *  - cron.test.js
 *  - productionHardeningNotifications.test.js
 *  - productionHardeningPdfWorker.test.js
 */
...
describe('scheduled Tasks And Workers Consolidated Suite', () => {
  afterAll(async () => {
    try {
    ...
    describe('Phase 6 — Background Cron Jobs Integration Tests', () => {
      ...
      it('4. should send due reminders once for loans exactly N days out, and not double-send on subsequent daily runs', async () => {
        ...
        loan.dueDate = new Date(
          Date.now() + (config.dueReminderDaysBefore - 1) * 24 * 60 * 60 * 1000
        );
        await loan.save();
  ...
  describe('[Source: productionHardeningNotifications.test.js]', () => {
    ...
    describe('ITEM 2 — Real Notification Delivery (Email/Push), Device Tokens & Delivery Logs', () => {
      ...
      test('2.2 Verify retry-and-log-failure behavior when provider throws', async () => {
        ...
        const failedLogsInDb = await NotificationLog.find({
          userId: testUser._id,
          status: 'failed',
        });
  ...
  describe('[Source: productionHardeningPdfWorker.test.js]', () => {
    const fs = require('fs');
