/**
 * Consolidated Suite: realtime And Web Sockets
 * Merged from:
 *  - realtime.test.js
 *  - socketCollegeRoom.test.js
 */
...
describe('realtime And Web Sockets Consolidated Suite', () => {
  afterAll(async () => {
    try {
  ...
  describe('[Source: realtime.test.js]', () => {
    ...
    describe('Phase 5 — Real-Time, Gamification & Notifications Integration Tests', () => {
      ...
      describe('Notification Service & Muting Preferences', () => {
        ...
        it('4. should skip creating Notification document entirely if the user has muted that type in preferences', async () => {
          ...
          // Check DB
          const dbAllowed = await Notification.findOne({
            userId: studentA._id,
            type: 'hold_ready',
          });
  ...
  describe('[Source: socketCollegeRoom.test.js]', () => {
    process.env.NODE_ENV = 'test';
